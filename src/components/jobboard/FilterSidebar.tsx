...
export default function FilterSidebar() {
  ...
  return (
    <aside className="md:sticky md:top-20">
      {/* Mobile trigger */}
      <button
        className="md:hidden w-full mb-3 px-4 py-2 rounded-xl border border-gray-200 text-gray-700 flex items-center justify-between"
        onClick={() => setOpen((v) => !v)}
      >
      ...
      <div className={`rounded-2xl border border-gray-200 bg-white p-4 ${open ? "block" : "hidden md:block"}`}>
        <div className="mb-4">
        ...
        <div>
          <button className="w-full px-4 py-2 rounded-xl bg-[#0D6EFD] text-white font-medium hover:opacity-90">Apply Filters</button>
        </div>
      </div>
    </aside>
  );
