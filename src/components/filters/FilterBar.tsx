...
export function FilterBar({ onApply }: { onApply: (vals: { keyword?: string; category?: string; location?: string; sort?: string }) => void }) {
  ...
  return (
    <div className="bg-card text-card-foreground p-4 border border-border rounded-xl grid gap-3 md:grid-cols-5">
      ...
      <button onClick={() => onApply({ keyword, category, location, sort })} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:opacity-95">Apply</button>
    </div>
  );
}
