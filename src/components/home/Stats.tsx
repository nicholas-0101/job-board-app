...
export function Stats() {
  ...
  return (
    <section className="container mx-auto px-4 py-16">
      <motion.div 
        className="grid rounded-3xl border border-border bg-card/80 backdrop-blur-sm p-8 md:p-12 md:grid-cols-3 text-center shadow-xl"
      ...
      >
        {stats.map((s, i) => {
          ...
          return (
            <motion.div 
              key={s.label} 
              className={`py-6 ${i !== stats.length - 1 ? "md:border-r border-border" : ""}`}
