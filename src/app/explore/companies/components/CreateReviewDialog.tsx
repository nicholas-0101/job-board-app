...
export default function CreateReviewDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (ratings.some(rating => rating === 0 || rating < 0.1)) {
      toast.error("Please provide all ratings (minimum 0.1)");
      return;
    }
    ...
    try {
      const payload = {
        ...
        salaryEstimateMin: formData.salaryEstimateMin ? parseInt(formData.salaryEstimateMin) : null,
        salaryEstimateMax: formData.salaryEstimateMax ? parseInt(formData.salaryEstimateMax) : null,
        body: formData.body,
      };
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              <input
                ...
                className={`w-16 px-2 py-1 text-sm border rounded text-center transition-all duration-200 ${
                  rating > 0 
                    ? "border-[#467EC7] bg-[#467EC7]/5 text-[#467EC7] font-medium" 
                    : "border-gray-300 hover:border-gray-400"
                } focus:ring-2 focus:ring-[#467EC7] focus:border-[#467EC7]`}
                placeholder="0.0"
                title="Enter a rating between 0.1 and 5.0"
              />
              {rating > 0 && (
                <div className="absolute -top-1 -right-1 w-2 h-2 bg-[#24CFA7] rounded-full animate-pulse"></div>
              )}
            </div>
            ...
            {!isAuthenticated ? (
              ...
              {userEmployment && (
                <div className="bg-[#E1F1F3] border border-[#467EC7]/20 rounded-lg p-4">
                  <h4 className="font-medium text-[#467EC7] mb-2">Your Employment at {company.name}</h4>
                  <div className="text-sm text-[#467EC7] space-y-1">
                    <p><strong>Position:</strong> {userEmployment.positionTitle}</p>
                    <p><strong>Duration:</strong> {new Date(userEmployment.startDate).toLocaleDateString()} - {userEmployment.isCurrent ? 'Present' : userEmployment.endDate ? new Date(userEmployment.endDate).toLocaleDateString() : 'Present'}</p>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData(prev => ({ ...prev, positionTitle: e.target.value }))
                  }
                ...
                {userEmployment && (
                  <p className="text-xs text-gray-500 mt-1">
                    Pre-filled from your employment record. You can modify if needed.
                  </p>
                )}
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-gray-900">Anonymous Review</p>
                    <p className="text-sm text-gray-500">
                      {formData.isAnonymous 
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <InteractiveStarRating
                    ...
                    onRatingClick={(rating) => handleRatingClick("ratingCulture", rating)}
                    label="Company Culture"
                  />
                  <InteractiveStarRating
                    rating={formData.ratingFacilities}
                    onRatingClick={(rating) => handleRatingClick("ratingFacilities", rating)}
                    label="Facilities & Benefits"
                  />
                  <InteractiveStarRating
                    rating={formData.ratingWorkLife}
                    onRatingClick={(rating) => handleRatingClick("ratingWorkLife", rating)}
                    label="Work-Life Balance"
                  />
                  <InteractiveStarRating
                    rating={formData.ratingCareer}
                    onRatingClick={(rating) => handleRatingClick("ratingCareer", rating)}
                    label="Career Growth"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData(prev => ({ ...prev, salaryEstimateMin: e.target.value }))
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData(prev => ({ ...prev, salaryEstimateMax: e.target.value }))
                    }
