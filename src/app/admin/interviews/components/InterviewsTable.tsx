...
export function InterviewsTable({ items, onEdit, onCancel }: InterviewsTableProps) {
  return (
            ...
            {items.length === 0 ? (
              ...
              items.map((it) => (
                <tr key={it.id} className="hover:bg-secondary/50 transition-colors">
                  ...
                  <td className="p-4">
                    {it.status === 'SCHEDULED' && (
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs bg-blue-100 text-blue-700 font-medium">📅 Scheduled</span>
                    )}
                    {it.status === 'COMPLETED' && (
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs bg-green-100 text-green-700 font-medium">✓ Completed</span>
                    )}
                    {it.status === 'CANCELLED' && (
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs bg-red-100 text-red-700 font-medium">✕ Cancelled</span>
                    )}
                    {it.status === 'NO_SHOW' && (
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs bg-gray-100 text-gray-700 font-medium">No Show</span>
                    )}
                  </td>
                  <td className="p-4">
                    <div className="flex gap-2">
                      <button onClick={() => onEdit(it)} className="px-3 py-1.5 text-sm border border-blue-300 rounded-xl hover:bg-blue-50 text-blue-600 transition-colors flex items-center gap-1">
                      ...
                      <button onClick={() => onCancel(it.id)} className="px-3 py-1.5 text-sm border border-red-300 rounded-xl hover:bg-red-50 text-red-600 transition-colors">Cancel</button>
                    </div>
                  </td>
                </tr>
              ))
